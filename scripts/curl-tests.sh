...
curl -fsS "$BASE/api/workers/status" | node -e 'let s="";process.stdin.on("data",d=>s+=d);process.stdin.on("end",()=>{const j=JSON.parse(s); const c=j.workers?.codex; const a=j.workers?.claude; if(!c?.mode || !a?.mode || !c?.requestedMode || !a?.requestedMode || typeof c.available!=="boolean" || typeof a.available!=="boolean" || typeof c.ready!=="boolean" || typeof a.ready!=="boolean" || typeof j.cwdAvailable!=="boolean" || typeof j.timeoutMs!=="number") process.exit(1); console.log(`codex=${c.mode}/${c.requestedMode} ready=${c.ready} claude=${a.mode}/${a.requestedMode} ready=${a.ready} cwd=${j.cwdAvailable}`)})'

echo "worker env gate direct"
DOJO_WORKER_MODE=cli DOJO_CODEX_BIN=/bin/echo DOJO_CLAUDE_BIN=/bin/echo node --input-type=module -e 'const { workerStatus } = await import("./src/workers.js"); const s = workerStatus(); if (s.workers.codex.mode !== "cli" || s.workers.claude.mode !== "cli" || !s.workers.codex.available || !s.workers.claude.available) process.exit(1); console.log(`global=${s.workers.codex.mode}/${s.workers.claude.mode}`);'
DOJO_CODEX_WORKER=bogus DOJO_CLAUDE_WORKER=cli DOJO_CODEX_BIN=/bin/echo DOJO_CLAUDE_BIN=/bin/echo node --input-type=module -e 'const { workerStatus } = await import("./src/workers.js"); const s = workerStatus(); if (s.workers.codex.mode !== "stub" || s.workers.codex.requestedMode !== "bogus" || !s.workers.codex.warning || s.workers.claude.mode !== "cli") process.exit(1); console.log(`per_agent=${s.workers.codex.mode}/${s.workers.claude.mode}`);'
